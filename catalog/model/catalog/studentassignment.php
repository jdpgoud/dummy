...
class ModelCatalogStudentassignment extends Model {
	...
	public function getProduct($product_id) {
		...
		$results=$this->getListOfProductTopics($query->row['product_id']);
	...
	$results1=$this->getListOfTopicSessions($data['topic'][0]['topic_id']);
		 foreach($results1 as $result1){
				$data['topicsessions'][] = array(
				'session_id'  => $result1['session_id'],
				'title'       => $result1['title'], 
				'test_id'	 => $result1['test_id'], 
				'topic_id'	=> $result1['topic_id'], 
				'assignment_file' => $result1['assignment_file']
                );
				
		 }
		 
		 $SessionVideosresults=$this->getListOfSessionVideos($data['topicsessions'][0]['session_id']);
